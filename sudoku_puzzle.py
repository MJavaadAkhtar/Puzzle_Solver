...
class SudokuPuzzle(Puzzle):
    ...
    def fail_fast(self):
        ...
        new_list = []
        for i in range(n):
            new_list.append(self._symbols[ i*n : n*(i+1) ] )
        rows = []
        for row in new_list:
            new_row = []
            for i in range(len(row)):
                ...
                if s !="*":
                    new_row.append(s)
            for k in self._symbol_set:
                if new_row.count(k) >= 2:
                    return True
            rows.append(new_row)
        column_list = []
        for j in range(n):
            ...
            for i in range(n):
                if new_list[i][j] != "*":
                    new_column.append(new_list[i][j])
            column_list.append(new_column)
            for h in self._symbol_set:
                if new_column.count(h) >= 2:
                    return True
        subsquare_list = []
        for m in range(n):
            jj =m%sqrt(n)
            ...
            for i in range(int(sqrt(n))):
                for j in range(int(sqrt(n))):
                    s =new_list[int( i + ii * sqrt(n))][int(j + jj * sqrt(n))]
                    if s != "*":
                        subsquare.append(s)
            for y in self._symbol_set:
                if subsquare.count(y) >= 2:
                    return True
            subsquare_list.append(subsquare)
            
        for i in range(n):
            for j in range(n):
                if new_list[i][j] == "*":
                    symbols_set =set(rows[i] + column_list[j] + subsquare_list[int(j//sqrt(n) + (i//sqrt(n))*sqrt(n))])
                    if symbols_set == self._symbol_set:
                        return True
        return False
